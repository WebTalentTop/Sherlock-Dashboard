...
var Timeline = (function () {
    ...
    Timeline.prototype.addBrush = function () {
        ...
        this.brush = d3_brush_1.brushX()
            .extent([[0, 0], [width, height]])
            .on('brush end', function () {
            _this.zone.run(function () {
                var selection = d3_selection_1.event.selection || _this.xScale.range();
                var newDomain = selection.map(_this.xScale.invert);
                _this.onDomainChange.emit(newDomain);
                _this.cd.markForCheck();
            });
        });
        d3_selection_1.select(this.element)
            .select('.brush')
            .call(this.brush);
    };
    Timeline.prototype.updateBrush = function () {
        ...
        this.zone.run(function () {
            ...
            d3_selection_1.select(_this.element)
                .select('.brush')
                .call(_this.brush);
            // clear hardcoded properties so they can be defined by CSS
            d3_selection_1.select(_this.element).select('.selection')
                .attr('fill', undefined)
