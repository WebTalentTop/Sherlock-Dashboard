...
var ColorHelper = (function () {
    ...
    ColorHelper.prototype.generateColorScheme = function (scheme, type, domain) {
        ...
        var colorScale;
        if (type === 'quantile') {
            colorScale = d3_scale_1.scaleQuantile()
                .range(scheme.domain)
                .domain(domain);
        }
        else if (type === 'ordinal') {
            colorScale = d3_scale_1.scaleOrdinal()
                .range(scheme.domain)
                .domain(domain);
        }
        else if (type === 'linear') {
            colorScale = d3_scale_1.scaleLinear()
                .domain(d3_array_1.range(0, 1, 1.0 / (scheme.domain.length - 1)))
                .range(scheme.domain);
        }
        return colorScale;
    };
    ColorHelper.prototype.getColor = function (value) {
        if (this.scaleType === 'linear') {
            var valueScale = d3_scale_1.scaleLinear()
                .domain(this.domain)
                .range([0, 1]);
    ...
    ColorHelper.prototype.getLinearGradientStops = function (value, start) {
        ...
        var valueScale = d3_scale_1.scaleLinear()
            .domain(this.domain)
            .range([0, 1]);
        var colorValueScale = d3_scale_1.scaleBand()
